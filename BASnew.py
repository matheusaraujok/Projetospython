...
def buscar_dados(ticker: str):
    try:
        # Adiciona .SA automaticamente para ativos da B3
        ticker_completo = f"{ticker}.SA"
        dados = yf.download(ticker_completo, period="5d", interval="15m", progress=False)
        if dados.empty:
...
app = tk.Tk()
app.title("Análise de Ativos B3")
app.geometry("300x200")

# Título
titulo = tk.Label(app, text="Analisador de Ativos B3", font=("Arial", 14))
titulo.pack(pady=10)
